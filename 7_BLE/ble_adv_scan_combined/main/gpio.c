 ...
 * @retval None
 */
void Run_States(uint8_t *interrupt_flag, uint8_t *pled_status){
    #define MAX_STATE   3
    ...
    *interrupt_flag = 0;  // 0 Led Desligado // 1 Led Ligado // 2 Led Piscando
    ...
    while(1){
        ...
        if(*pled_status != *interrupt_flag){
            //ESP_LOGI(TAG, "Uptating BLE ADV");
            *pled_status = *interrupt_flag;
            //hci_cmd_send_ble_set_adv_data(*pled_status);
            ble_adv_set_data(pled_status, 1);
        }
...
void gpio_main(void *pvParameters)
{
    uint8_t pled_status = 0;
    uint8_t interrupt_flag = 0;
    ESP_LOGI(TAG, "Configuring GPIOs");
    Pin_As_Output(BLUE_LED);
    Pin_As_IRAM_Input(BUTTON, &interrupt_flag);
    ESP_LOGI(TAG, "Starting States");
    Run_States(&interrupt_flag, &pled_status);
}
